    ...
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            responseCharges = new List<ResponseItem>(list.Select(x => new ResponseItem
            {
                ...
                ChargeGroupName = x.ChargeGroupName,
                ...
                ChargesDetails = x.ChargesDetails
            }));
        ...
        {
            //var query = _context.Charges.AsQueryable().Where(x => x.IsDeposit == false);
            var query = (from c in _context.Charges
                         where !c.IsDeposit
                         select new Entities.Charges
                         {
                             ChargesId = c.ChargesId,
                             ChargesCode  = c.ChargesCode,
                             ChargesName = c.ChargesName,
                             TransactionType = c.TransactionType,
                             ChargeGroupCode = c.ChargeGroupCode,
                             ChargeGroupName = (!string.IsNullOrEmpty(c.ChargeGroupCode) ? _context.ChargeGroups.Where(x=>x.ChargeGroupCode == c.ChargeGroupCode).Select(s=>s.ChargeGroupName).FirstOrDefault() : ""),
                             IsPurchasing = c.IsPurchasing,
                             IsSales = c.IsSales,
                             IsAsset = c.IsAsset,
                             IsFinancial = c.IsFinancial,
                             IsInventory = c.IsInventory,
                             IsDeposit = c.IsDeposit,
                             RevenueAccountNo = c.RevenueAccountNo,
                             CostAccountNo = c.CostAccountNo,
                             TempRevenueAccountNo = c.TempRevenueAccountNo,
                             TaxScheduleCode = c.TaxScheduleCode,
                             ShippingLineType = c.ShippingLineType,
                             HasCosting = c.HasCosting,
