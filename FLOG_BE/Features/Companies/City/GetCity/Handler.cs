...
namespace FLOG_BE.Features.Companies.City.GetCity
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var query = getCity(request.Initiator.UserId, request.Filter);
            query = getCitySorted(query, request.Sort);

            var list = await PaginatedList<Entities.City, ResponseItem>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<Entities.City> getCity(string personId, RequestFilter filter)
        {
            var query = _context.Cities
                    .Include(x => x.Country)
                     .AsQueryable();

            var filterCode = filter.CityCode?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterCode.Any())
            {
                var predicate = PredicateBuilder.New<Entities.City>(true);
            ...
            var filterName = filter.CityName?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterName.Any())
            {
                var predicate = PredicateBuilder.New<Entities.City>(true);
                foreach (var filterItem in filterName)
                {
                    predicate = predicate.Or(x => x.CityName.Contains(filterItem, StringComparison.OrdinalIgnoreCase));
                }
                query = query.Where(predicate);
            }

            var filterProvince = filter.Province?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterProvince.Any())
            {
                var predicate = PredicateBuilder.New<Entities.City>(true);
                foreach (var filterItem in filterProvince)
                {
                    predicate = predicate.Or(x => x.Province.Contains(filterItem, StringComparison.OrdinalIgnoreCase));
                }
                query = query.Where(predicate);
            }
            ...
            if (filterCountryName.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => x.Country.CountryName.Contains(filterItem, StringComparison.OrdinalIgnoreCase));
                }
                query = query.Where(predicate);
            }
        ...
        private IQueryable<Entities.City> getCitySorted(IQueryable<Entities.City> input, List<string> sort)
        {
            ...
            foreach (var item in sortingList)
            {
                ...
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.CityName) : query.ThenBy(x => x.CityName);
                }

                if (item.Contains("Province", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.Province) : query.ThenBy(x => x.Province);
                }
                if (item.Contains("CountryName", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.Country.CountryName) : query.ThenBy(x => x.Country.CountryName);
                }
            ...
            if (!sortingList.Any())
            {
                query = query.ThenBy(x => x.CityCode);
            }
