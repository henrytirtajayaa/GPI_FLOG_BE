using System;
using System.Collections.Generic;
using Infrastructure.Mediator;

namespace FLOG_BE.Features.Companies.ExchangeRateHeader.GetExchangeRateHeader
{
    public class Request : IRequest<Response>
    {
        public UserLogin Initiator { get; set; }
        public RequestFilter Filter { get; set; }
        public Int32 Offset { get; set; }
        public Int32 Limit { get; set; }
        public List<string> Sort { get; set; }
    }

    public class RequestFilter
    {
        public List<string> ExchangeRateCode { get; set; }
        public List<string> Description { get; set; }
        public List<string> CurrencyCode { get; set; }
        public List<int?> RateTypeMin { get; set; }
        public List<int?> RateTypeMax { get; set; }
        public List<string> ExpiredPeriod { get; set; }
        public List<int?> CalculationTypeMin { get; set; }
        public List<int?> CalculationTypeMax { get; set; }
        public List<int?> StatusMin { get; set; }
        public List<int?> StatusMax { get; set; }
        public List<string> CreatedBy { get; set; }
        public List<DateTime?> CreatedDateStart { get; set; }
        public List<DateTime?> CreatedDateEnd { get; set; }
        public List<DateTime?> ModifiedDateStart { get; set; }
        public List<DateTime?> ModifiedDateEnd { get; set; }
        public List<string> ModifiedBy { get; set; }
        public List<DateTime?> ModifiedDate { get; set; }
    }
}
