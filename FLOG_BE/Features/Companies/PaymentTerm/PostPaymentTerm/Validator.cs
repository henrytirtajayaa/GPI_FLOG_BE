...
namespace FLOG_BE.Features.Companies.PaymentTerm.PostPaymentTerm
{
    public class Validator : AsyncRequestValidator<Request>
    {
        ...
        protected override async Task<ValidationResult> Validate(Request request)
        {
            ...
            if (!request.Body.Due.HasValue)
                return ValidationResult.ValidationError($"{nameof(request.Body.Due)} cannot be null");

            if (!request.Body.Unit.HasValue)
                return ValidationResult.ValidationError($"{nameof(request.Body.Unit)} cannot be null");
