...
namespace FLOG_BE.Features.Companies.Port.GetPort
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var query = getPort(request.Initiator.UserId, request.Filter);
            query = getPortSorted(query, request.Sort);

            var list = await PaginatedList<Entities.Port, ResponseItem>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<Entities.Port> getPort(string personId, RequestFilter filter)
        {
            var query = _context.Ports
                .Include(x => x.Cities)
                .Include(x => x.Cities.Country)
                .AsQueryable();

            var filterPortCode = filter.PortCode?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterPortCode.Any())
            {
                var predicate = PredicateBuilder.New<Entities.Port>(true);
                foreach (var filterItem in filterPortCode)
                {
                    predicate = predicate.Or(x => x.PortCode.ToLower().Contains(filterItem.ToLower()));
                }
                query = query.Where(predicate);
            }

            var filterPortName = filter.PortName?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterPortName.Any())
            {
                var predicate = PredicateBuilder.New<Entities.Port>(true);
                foreach (var filterItem in filterPortName)
                {
                    predicate = predicate.Or(x => x.PortName.ToLower().Contains(filterItem.ToLower()));
                }
                query = query.Where(predicate);
            }

            var filterCityCode = filter.CityName?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            ...
            var filterPortType = filter.PortType?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterPortType.Any())
            {
                var predicate = PredicateBuilder.New<Entities.Port>(true);
                foreach (var filterItem in filterPortType)
                {
                    predicate = predicate.Or(x => x.PortType.Contains(filterItem));
                }
                query = query.Where(predicate);
            }
            return query;
        ...
        private IQueryable<Entities.Port> getPortSorted(IQueryable<Entities.Port> input, List<string> sort)
        {
            ...
            {
                if (item.Contains("PortCode", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.PortCode) : query.ThenBy(x => x.PortCode);
                }

                if (item.Contains("PortName", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.PortName) : query.ThenBy(x => x.PortName);
                }
                if (item.Contains("PortType", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.PortType) : query.ThenBy(x => x.PortType);
                }
            ...
            if (!sortingList.Any())
            {
                query = query.ThenBy(x => x.PortCode);
            }
