...
namespace FLOG_BE.Features.Companies.VendorAddress.GetVendorAddress
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var query = getVendorAddress(request.Initiator.UserId, request.Filter);

            List<Person> ListUser = await GetUser();

            query = getVendorAddressSorted(query, request.Sort);
            var list = await PaginatedList<Entities.VendorAddress, ResponseItem>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
            ...
            {
               VendorId = x.VendorId,
                VendorAddressId = x.VendorAddressId,
            ...
            {
                VendorAddresses = response,
                ListInfo = list.ListInfo
            });
        }
        private IQueryable<Entities.VendorAddress> getVendorAddress(string personId, RequestFilter filter)
        {
            var query = (from addr in _context.VendorAddresses
                         select new Entities.VendorAddress
                         {
                             VendorAddressId = addr.VendorAddressId,
                             ...
                             City = (from mycity in _context.Cities
                                     where mycity.CityCode == addr.CityCode 
                                     select mycity).FirstOrDefault(),
                             Vendor = (from myvend in _context.Vendors
                                         where myvend.VendorId == addr.VendorId
                                       select myvend).FirstOrDefault()
            ...
            {
                var predicate = PredicateBuilder.New<Entities.VendorAddress>(true);
            ...
            if (filterVendorCode.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => x.Vendor.VendorCode.ToLower().Contains(filterItem.ToLower()));
                }
                query = query.Where(predicate);
            }
            ...
            if (filterVendorName.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => x.Vendor.VendorName.ToLower().Contains(filterItem.ToLower()));
                }
                query = query.Where(predicate);
            }
            ...
            if (filterName.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => string.Compare(x.AddressName, filterItem, StringComparison.OrdinalIgnoreCase) >= 0);
                }
                query = query.Where(predicate);
            }
            ...
            if (filterCreatedBy.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => x.CreatedBy.ToLower().Contains(filterItem));
                }
                query = query.Where(predicate);
            }
            ...
            if (filterModifiedBy.Any())
            {
                ...
                {
                    predicate = predicate.Or(x => x.ModifiedBy.ToLower().Contains(filterItem));
                }
                query = query.Where(predicate);
            }
        ...
        private IQueryable<Entities.VendorAddress> getVendorAddressSorted(IQueryable<Entities.VendorAddress> input, List<string> sort)
        {
            var query = input.OrderBy(x => 0);
            var sortingList = sort.Where(x => !string.IsNullOrEmpty(x)).Select(x => x.TrimStart());
            foreach (var item in sortingList)
            {
                if (item.Contains("VendorAddressId", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.VendorAddressId) : query.ThenBy(x => x.VendorAddressId);
                } 
                if (item.Contains("VendorCode", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.Vendor.VendorCode) : query.ThenBy(x => x.Vendor.VendorCode);
                }
                if (item.Contains("VendorName", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.Vendor.VendorName) : query.ThenBy(x => x.Vendor.VendorName);
                }
                ...
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.City.CityName) : query.ThenBy(x => x.City.CityName);
                }
            ...
            if (!sortingList.Any(x => x.Contains("AddressCode", StringComparison.InvariantCultureIgnoreCase)))
            {
