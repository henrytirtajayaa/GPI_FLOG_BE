...
namespace FLOG_BE.Features.Finance.ARApply.GetProgressApplyReceivable
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        {
            IQueryable<Entities.ARApplyHeader> query = new List<Entities.ARApplyHeader>().AsQueryable();
            ...
            try
            {
                ...
                List<ReceivableTransactionHeader> invoices = _context.ReceivableTransactionHeaders.ToList();

                var queryReceipts = _context.ArReceiptHeaders.Where(x => x.Status == DOCSTATUS.POST).AsQueryable();
                ...
                var availablesAdvances = _context.ARPendingAdvanceReceipts.AsQueryable();
                var availablesCreditNotes = _context.ARPendingCreditNotes.AsQueryable();
                var unapplyReceipts = _context.ARUnapplyReceipts.AsQueryable();

                query = (from x in _context.ARApplyHeaders
                         ...
                         select new Entities.ARApplyHeader
                         {
                             ...
                             (x.DocumentType.Equals(DOCUMENTTYPE.CREDIT_NOTE) ? (availablesCreditNotes.Where(c => c.ReceiveTransactionId == x.ReceiveTransactionId).Select(o => o.OriginatingBalance).FirstOrDefault()) :
                               (unapplyReceipts.Where(c => c.ReceiptHeaderId == x.ReceiptHeaderId).Select(o => o.OriginatingBalance).FirstOrDefault()))
            ...
            {
                Console.WriteLine("[getTransaction] *********** " + ex.Message);
                Console.WriteLine("[getTransaction] *********** " + ex.StackTrace);
            }
