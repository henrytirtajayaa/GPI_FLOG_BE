    ...
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    var entryHeader = await _context.ARApplyHeaders.FirstOrDefaultAsync(x => x.ReceivableApplyId == request.Body.ReceivableApplyId);
                    ...
                    if(entryHeader != null)
                    {
                        ...
                        if (request.Body.ActionDocStatus == DOCSTATUS.DELETE)
                        {
                            if(entryHeader.Status == DOCSTATUS.NEW)
                            {
                                ...
                                _context.ARApplyHeaders.Update(entryHeader);
                        ...
                        }else if(request.Body.ActionDocStatus == DOCSTATUS.POST)
                        {
                            if (entryHeader.Status == DOCSTATUS.NEW)
                            {
                                ...
                                var validJournal = await _financeManager.PostJournalAsync(entryHeader.DocumentNo, TRX_MODULE.TRX_APPLY_RECEIPT, request.Initiator.UserId, entryHeader.TransactionDate);
                        ...
                        else if(request.Body.ActionDocStatus == DOCSTATUS.VOID)
                        {
                            if (entryHeader.Status == DOCSTATUS.POST)
                            {
                                ...
                                //UPDATE AR RECEIPT DETAIL REFF
                                var arReceiptDetails = _context.ArReceiptDetails.Where(x=>x.ReceiptHeaderId == entryHeader.ReceiptHeaderId);

                                foreach(var ard in arReceiptDetails)
                                {
                                    ard.Status = DOCSTATUS.VOID;
                                    
                                    _context.ArReceiptDetails.Update(ard);
                                }

                                //DO SOME POSTING TO REVERSE JOURNAL WITH SAME DOCUMENT
                                var resp = await _financeManager.VoidJournalAsync(entryHeader.DocumentNo, TRX_MODULE.TRX_APPLY_RECEIPT, request.Initiator.UserId, request.Body.ActionDate);
                ...
                catch (Exception ex)
                {
                    ...
                    return ApiResult<Response>.InternalServerError("[PutStatusApplyReceivable] " + ex.Message);
                }
            }
        }
    }
