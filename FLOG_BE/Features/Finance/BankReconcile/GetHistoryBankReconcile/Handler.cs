...
namespace FLOG_BE.Features.Finance.BankReconcile.GetHistoryBankReconcile
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.BankReconcileHeader> getTransactions(string personId, RequestFilter filter)
        {
            ...
            List<BankReconcileHeader> reconcileNewPost = _context.BankReconcileHeaders.Where(x => x.Status == DOCSTATUS.POST || x.Status == DOCSTATUS.NEW).ToList();
            ...
            var query = (from x in _context.BankReconcileHeaders
                         ...
                         select new Entities.BankReconcileHeader
                         {
                             ...
                             AllowVoid = (x.Status == DOCSTATUS.POST ?
                                         (reconcileNewPost.Where(b=>b.PrevBankReconcileId==x.BankReconcileId).Any() ? false : true) : false),
            ...
            {
                x.ReconcileDetails = (from ba in _financeCatalog.GetBankReconcileDetailById(x.BankReconcileId)
                                      where ba.BankReconcileId == x.BankReconcileId
