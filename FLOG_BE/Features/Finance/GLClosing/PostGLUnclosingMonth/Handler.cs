...
namespace FLOG_BE.Features.Finance.GLClosing.PostGLUnclosingMonth
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                {
                    var closing = await _financeManager.UncloseMonth(funcCurrencyCode, request.Body.PeriodYear, request.Body.PeriodIndex, request.Initiator.UserId);
                    ...
                    if (closing.Valid)
                    {
                        ...
                        {
                            Message = string.Format("Unclose Period Of {0}/{1} success !", request.Body.PeriodYear, request.Body.PeriodIndex)
                        });
                    }
                    else
                    {
