...
namespace FLOG_BE.Features.Finance.JournalEntry.GetHistoryJournalEntry
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.JournalEntryHeader> GetTransactions(string personId, RequestFilter filter)
        {
            ...
            var query = (from x in _context.JournalEntryHeaders
                         ...
                         where x.Status != DOCSTATUS.NEW && x.Status != DOCSTATUS.PROCESS && x.Status != DOCSTATUS.APPROVE  && x.Status != DOCSTATUS.REVISE
                         select new Entities.JournalEntryHeader
                         {
                             ...
                             JournalEntryDetails = (from dets in _context.JournalEntryDetails
                                                    where dets.JournalEntryHeaderId == x.JournalEntryHeaderId
                                                    select dets).ToList(),
                             VoidBy = ListUser.Where(p => p.PersonId == x.VoidBy).Select(p => p.PersonFullName).FirstOrDefault(),
                             VoidDate = x.VoidDate,
