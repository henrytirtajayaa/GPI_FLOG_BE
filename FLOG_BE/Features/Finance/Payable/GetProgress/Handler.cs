...
namespace FLOG_BE.Features.Finance.Payable.GetProgress
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.PayableTransactionHeader> getTransaction(string personId, RequestFilter filter)
        {
            ...
            var filterVendorDocumentNo = filter.VendorDocumentNo?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterVendorDocumentNo.Any())
            {
                var predicate = PredicateBuilder.New<Entities.PayableTransactionHeader>(false);
                foreach (var filterItem in filterVendorDocumentNo)
                {
