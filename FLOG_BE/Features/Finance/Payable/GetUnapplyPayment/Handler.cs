...
namespace FLOG_BE.Features.Finance.Payable.GetUnapplyPayment
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var list = await PaginatedList<APUnapplyPayment, APUnapplyPayment>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<APUnapplyPayment> getTransaction(string personId, RequestFilter filter)
        {
            var query = _context.APUnapplyPayments.Where(x => x.VendorId == filter.VendorId)
                .AsNoTracking().AsEnumerable().ToList().AsQueryable();
            
            var wherePredicates = PredicateBuilder.New<APUnapplyPayment>(true);
                        
            if (!string.IsNullOrEmpty(filter.DocumentNo))
            {
                var predicate = PredicateBuilder.New<APUnapplyPayment>(false);
        ...
        private IQueryable<APUnapplyPayment> getReceivableSorted(IQueryable<APUnapplyPayment> input, List<string> sort)
        {
