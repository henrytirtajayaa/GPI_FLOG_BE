...
namespace FLOG_BE.Features.Finance.Receivable.GetAdvanceReceipt
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var list = await PaginatedList<ARAdvanceReceipt, ARAdvanceReceipt>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<ARAdvanceReceipt> getTransaction(string personId, RequestFilter filter)
        {
            var query = _context.ARPendingAdvanceReceipts.Where(x => x.CustomerId == filter.CustomerId)
                .AsNoTracking().AsEnumerable().ToList().AsQueryable();
            
            var wherePredicates = PredicateBuilder.New<ARAdvanceReceipt>(true);
                        
            if (!string.IsNullOrEmpty(filter.DocumentNo))
            {
                var predicate = PredicateBuilder.New<ARAdvanceReceipt>(false);
        ...
        private IQueryable<ARAdvanceReceipt> getSorted(IQueryable<ARAdvanceReceipt> input, List<string> sort)
        {
