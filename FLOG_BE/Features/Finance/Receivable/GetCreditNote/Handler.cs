...
namespace FLOG_BE.Features.Finance.Receivable.GetCreditNote
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var list = await PaginatedList<ARCreditNote, ARCreditNote>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<ARCreditNote> getTransaction(string personId, RequestFilter filter)
        {
            var query = _context.ARPendingCreditNotes.Where(x => x.CustomerId == filter.CustomerId)
                .AsNoTracking().AsEnumerable().ToList().AsQueryable();
            
            var wherePredicates = PredicateBuilder.New<ARCreditNote>(true);
                        
            if (!string.IsNullOrEmpty(filter.DocumentNo))
            {
                var predicate = PredicateBuilder.New<ARCreditNote>(false);
        ...
        private IQueryable<ARCreditNote> getReceivableSorted(IQueryable<ARCreditNote> input, List<string> sort)
        {
