...
namespace FLOG_BE.Features.Finance.Receivable.GetProgressDeposit
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.ReceivableTransactionHeader> getTransaction(string personId, RequestFilter filter)
        {
            ...
            var query = (from x in _context.ReceivableTransactionHeaders
                          ...
                          .Where(x => x.DocumentType == DOCUMENTTYPE.DEPOSIT_DEMURRAGE || x.DocumentType == DOCUMENTTYPE.CONTAINER_GUARANTEE || x.DocumentType == DOCUMENTTYPE.DETENTION)
                         select new Entities.ReceivableTransactionHeader
                         {
                             ...
                             SoDocumentNo = x.SoDocumentNo ?? string.Empty,
            ...
            var filterViodBy = filter.VoidBy?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterViodBy.Any())
            {
                var predicate = PredicateBuilder.New<Entities.ReceivableTransactionHeader>(false);
                foreach (var filterItem in filterViodBy)
                {
