...
namespace FLOG_BE.Features.Finance.Receivable.GetUnapplyReceipt
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var list = await PaginatedList<ARUnapplyReceipt, ARUnapplyReceipt>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());
        ...
        private IQueryable<ARUnapplyReceipt> getTransaction(string personId, RequestFilter filter)
        {
            var query = _context.ARUnapplyReceipts.Where(x => x.CustomerId == filter.CustomerId)
                .AsNoTracking().AsEnumerable().ToList().AsQueryable();
            
            var wherePredicates = PredicateBuilder.New<ARUnapplyReceipt>(true);
                        
            if (!string.IsNullOrEmpty(filter.DocumentNo))
            {
                var predicate = PredicateBuilder.New<ARUnapplyReceipt>(false);
        ...
        private IQueryable<ARUnapplyReceipt> getReceivableSorted(IQueryable<ARUnapplyReceipt> input, List<string> sort)
        {
