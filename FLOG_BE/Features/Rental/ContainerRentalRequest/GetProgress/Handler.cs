...
namespace FLOG_BE.Features.Rental.ContainerRentalRequest.GetProgress
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.ContainerRentalRequestHeader> getContainerRentalRequest(string personId, RequestFilter filter)
        {
            ...
            var query = (from x in _context.ContainerRentalRequestHeaders
                         .Where(x => x.Status != DOCSTATUS.SUBMIT && x.Status != DOCSTATUS.CANCEL && x.Status != DOCSTATUS.COMPLETE && x.Status != DOCSTATUS.DELETE && x.Status != DOCSTATUS.CLOSE)
                         select new Entities.ContainerRentalRequestHeader
                         {
                             ...
                             ContainerRentalRequestDetails = (from s in _context.ContainerRentalRequestDetails
