...
namespace FLOG_BE.Features.Rental.ContainerRequestConfirm.GetProgress
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        public async Task<ApiResult<Response>> Handle(Request request)
        {
            ...
            var query = getDoConfirm(request.Initiator.UserId, request.Filter);
            query = getDoConfirmSorted(query, request.Sort);

            List<Person> ListUser = await GetUser();
            List<Customer> ListCustomer = await GetCustomer();
            List<ContainerRentalRequestHeader> ListContainer = await GetContainerRentalHeader();
            List<ContainerRentalRequestDetail> ListDetail = await GetContainerRentalDetail();

            var list = await PaginatedList<Entities.ContainerRequestConfirmHeader, ResponseItem>.CreateAsync(_linkCollection, query, request.Offset, request.Limit, query.ToList().Count());

            return ApiResult<Response>.Ok(new Response()
            {
                ContainerRequestConfirm = list,
        ...
        private async Task<List<ContainerRentalRequestHeader>> GetContainerRentalHeader()
        {
            return await _context.ContainerRentalRequestHeaders.ToListAsync();
        }
        private async Task<List<ContainerRentalRequestDetail>> GetContainerRentalDetail()
        {
            return await _context.ContainerRentalRequestDetails.ToListAsync();
        }


        private IQueryable<Entities.ContainerRequestConfirmHeader> getDoConfirm(string personId, RequestFilter filter)
        {
            ...
            List<ContainerRentalRequestHeader> listContainer = _context.ContainerRentalRequestHeaders.ToList();
            List<ContainerRentalRequestDetail> listDetail = _context.ContainerRentalRequestDetails.ToList();

            var query = (from x in _context.ContainerRequestConfirmHeaders 
                         join c in _context.ContainerRentalRequestHeaders on x.ContainerRentalRequestHeaderId equals c.ContainerRentalRequestHeaderId
                         where x.Status != DOCSTATUS.SUBMIT && x.Status!= DOCSTATUS.CONFIRM && x.Status != DOCSTATUS.EXPIRE && x.Status != DOCSTATUS.CANCEL && x.Status != DOCSTATUS.DELETE && x.Status != DOCSTATUS.CLOSE
                         select new Entities.ContainerRequestConfirmHeader
                         {
                             ContainerRequestConfirmHeaderId = x.ContainerRequestConfirmHeaderId,
                             ContainerRentalRequestHeaderId = x.ContainerRentalRequestHeaderId,
                             TransactionType = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.TransactionType).FirstOrDefault(),
                             DocumentDate = x.DocumentDate,
                             DocumentNo = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.DocumentNo).FirstOrDefault(),
                             CustomerId = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.CustomerId).FirstOrDefault(),
                             CustomerCode = litdata.Where(p => p.CustomerId == c.CustomerId).Select(c => c.CustomerCode).FirstOrDefault(),
                             CustomerName = litdata.Where(p => p.CustomerId == c.CustomerId).Select(c => c.CustomerName).FirstOrDefault(),
                             AddressCode = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.AddressCode).FirstOrDefault(),
                             Status = x.Status,
                             SalesCode = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.SalesCode).FirstOrDefault(),
                             VendorId = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.VendorId).FirstOrDefault(),
                             VendorCode = litvendor.Where(p => p.VendorId == c.VendorId).Select(p => p.VendorCode).FirstOrDefault(),
                             VendorName = litvendor.Where(p => p.VendorId == c.VendorId).Select(p => p.VendorName).FirstOrDefault(),
                             BillToAddressCode = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.BillToAddressCode).FirstOrDefault(),
                             ShipToAddressCode = listContainer.Where(c => c.ContainerRentalRequestHeaderId == x.ContainerRentalRequestHeaderId).Select(c => c.ShipToAddressCode).FirstOrDefault(),
                             ...
                             ContainerRequestConfirmDetails = (from d in _context.ContainerRequestConfirmDetails
                                                               join rd in _context.ContainerRentalRequestDetails on d.ContainerRentalRequestDetailId equals rd.ContainerRentalRequestDetailId
                                                               where d.ContainerRequestConfirmHeaderId == x.ContainerRequestConfirmHeaderId
                                                               select new Entities.ContainerRequestConfirmDetail
                                                               {
                                                                   ...
                                                                   ContainerRentalRequestDetailId = rd.ContainerRentalRequestDetailId,
                                                                   ContainerCode = rd.ContainerCode,
            ...
            var wherePredicates = PredicateBuilder.New<Entities.ContainerRequestConfirmHeader>(true);

            var filterTransactionType = filter.TransactionType?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterTransactionType.Any())
            {
                var predicate = PredicateBuilder.New<Entities.ContainerRequestConfirmHeader>(false);
        ...
        private IQueryable<Entities.ContainerRequestConfirmHeader> getDoConfirmSorted(IQueryable<Entities.ContainerRequestConfirmHeader> input, List<string> sort)
        {
