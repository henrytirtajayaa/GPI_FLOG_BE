...
namespace FLOG_BE.Features.Sales.NegotiationSheet.GetNegotiationSheetApproved
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.NegotiationSheetHeader> getSalesData(string personId, RequestFilter filter)
        {
            ...
            var query = (from ng in _context.NegotiationSheetHeaders
                         ...
                         where ng.Status == DOCSTATUS.APPROVE
                         select new Entities.NegotiationSheetHeader
                         {
                             ...
                             NsContainers = new List<NegotiationSheetContainer>(),
                             NsTruckings = new List<NegotiationSheetTrucking>(),
                             NsSellings = (from sell in _context.NegotiationSheetSellings
                                           ...
                                           select new NegotiationSheetSelling
                                           {
                                               ...
                                               ReceiveTransactionId = sell.ReceiveTransactionId,
                                               ReceiveDocumentNo = _context.ReceivableTransactionHeaders.Where(iv=>iv.ReceiveTransactionId == sell.ReceiveTransactionId).Select(p=>p.DocumentNo).FirstOrDefault() ?? "",
                                           }).ToList(),
                             NsBuyings = (from buy in _context.NegotiationSheetBuyings
                                          ...
                                          where buy.NegotiationSheetId == ng.NegotiationSheetId
                                          ...
                                          select new Entities.NegotiationSheetBuying
                                          {
                                              ...
                                              PayableTransactionId = buy.PayableTransactionId,
                                              PayableDocumentNo = _context.PayableTransactionHeaders.Where(pay => pay.PayableTransactionId == buy.PayableTransactionId).Select(p => p.DocumentNo).FirstOrDefault() ?? "",
                                          }).Distinct().ToList(),
                             CurrentApprovers = new List<FLOG.Core.DocumentNo.TrxPersonApprover>()
                         }).AsEnumerable().ToList().AsQueryable();

            //CHECK PAYMENT CONDITION
            var trxTypes = _context.MSTransactionTypes.ToList();
            foreach (var ns in query)
            {
                int pCondition = trxTypes.Where(tt => tt.TransactionType == ns.TransactionType).Select(t => t.PaymentCondition).FirstOrDefault();

                bool isAnySellings = ns.NsSellings.Where(x => (x.PaymentCondition == pCondition || x.PaymentCondition == 0) && x.ReceiveTransactionId == Guid.Empty).Any();

                bool isAnyBuyings = ns.NsBuyings.Where(x => (x.PaymentCondition == pCondition || x.PaymentCondition == 0) && x.PayableTransactionId == Guid.Empty).Any();

                if (isAnySellings || isAnyBuyings)
                {
                    ns._cellClasses = "";
                    ns.IsInvoicePending = true;
                }
                else
                {
                    ns._cellClasses = "text-green";
                    ns.IsInvoicePending = false;
                }
            }
            ...
            if (filter.IsInvoicePending.HasValue)
            {
                var predicate = PredicateBuilder.New<Entities.NegotiationSheetHeader>(false);
                predicate = predicate.Or(x => x.IsInvoicePending == filter.IsInvoicePending);
                wherePredicates.And(predicate);
            }
        ...
        private IQueryable<Entities.NegotiationSheetHeader> getSorted(IQueryable<Entities.NegotiationSheetHeader> input, List<string> sort)
        {
            ...
            foreach (var item in sortingList)
            {
                ...
                if (item.Contains("IsInvoicePending", StringComparison.InvariantCultureIgnoreCase))
                {
                    query = item.StartsWith('-') ? query.ThenByDescending(x => x.IsInvoicePending) : query.ThenBy(x => x.IsInvoicePending);
                }
