...
namespace FLOG_BE.Features.Finance.Sales.Quotation.GetQuotationHistory
{
    public class Handler : IAsyncRequestHandler<Request, Response>
    {
        ...
        private IQueryable<Entities.SalesQuotationHeader> getQuotationHistory(string personId, RequestFilter filter)
        {
            ...
            var query = (from x in _context.SalesQuotationHeaders
                          .Where(x => x.Status == DOCSTATUS.CANCEL || x.Status == DOCSTATUS.CLOSE || x.Status == DOCSTATUS.SUBMIT)
            ...
            var filterTrasactionType = filter.TransactionType?.Where(x => !string.IsNullOrEmpty(x)).ToList();
            if (filterTrasactionType.Any())
            {
                var predicate = PredicateBuilder.New<Entities.SalesQuotationHeader>(false);
                foreach (var filterItem in filterTrasactionType)
                {
